...
def export_to_excel(data_list, filename="phone_link_analysis.xlsx"):
    ...
    """
    if not data_list:
        print("No data provided to export_to_excel function.")
        return
...
def test_scraper():
    ...
    test_excel_filename = "test_output_phone_analysis.xlsx"
    ...
    if not failures and test_processed_data:
        export_to_excel(test_processed_data, filename=test_excel_filename) # export_to_excel now expects 'status'
        if not os.path.exists(test_excel_filename):
            failures.append(f"Excel export failed: File '{test_excel_filename}' not created.")
        else:
            try:
                os.remove(test_excel_filename)
                print(f"Test Excel file '{test_excel_filename}' created and removed successfully.")
            except OSError as e:
                failures.append(f"Error removing test Excel file '{test_excel_filename}': {e}")
