(function() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function generatePassword() {
            ...
            // This check is common and ensures transformedWord is shorter than desiredLength if we proceed.
            ...
            passwordPatternRadios.forEach(radio => { if (radio.checked) selectedPattern = radio.value; });
            ...
            if (selectedPattern === 'append') {
                ...
                if (currentPassword.length > desiredLength) currentPassword = currentPassword.substring(0, desiredLength);
                    ...
                    let paddingSource = availableChars !== '' ? availableChars : lowercaseChars; // Fallback to lowercase if no types selected
                ...
                // Handle Empty availableChars if modifications are needed
                // (transformedWord is shorter than desiredLength here)
                ...
                if (passwordArray.length > 0 && availableChars !== '') {
                    const numReplacements = Math.min(passwordArray.length, 1); // Replace 1 char
                ...
                if (neededForPadding > 0) {
                    if (availableChars === '') { // Safeguard, should have been caught by the first check in this block
                    ...
                    // Ensure Number in insertPartArray (or passwordArray)
                    ...
                    if (charTypes.numbers && !currentFullStringForCheck.split('').some(c => numberChars.includes(c))) {
                        ...
                        } else if (passwordArray.length > 0) { // If insertPart is empty, try to change in passwordArray
                            passwordArray[Math.floor(Math.random() * passwordArray.length)] = numberChars.charAt(Math.floor(Math.random() * numberChars.length));
                        }
                    }

                    // Ensure Symbol in insertPartArray (or passwordArray)
                    currentFullStringForCheck = passwordArray.join('') + insertPartArray.join(''); // Recheck after potential number insertion
                ...
                if (passwordArray.length > desiredLength) {
                    ...
                    let paddingSource = availableChars !== '' ? availableChars : lowercaseChars; // Fallback
